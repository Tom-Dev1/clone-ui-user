...
export default function AgencyProductRequest() {
  ...
  const handleSubmitRequest = async () => {
    if (cartItems.length === 0) {
      toast.error(
        "Giỏ hàng trống. Vui lòng thêm sản phẩm trước khi gửi yêu cầu."
      );
      return;
    }
    ...
    if (Object.keys(quantityErrors).length > 0) {
      toast.error("Vui lòng sửa lỗi số lượng sản phẩm trước khi gửi yêu cầu.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        "https://minhlong.mlhr.org/api/request-products",
        {
          method: "POST",
          headers: {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        throw new Error(
          "Server trả về định dạng không hợp lệ. Vui lòng thử lại sau."
        );
      }
      ...
      toast.error("Bạn đã đặt hàng, Vui lòng thử lại sau 24h");
    } finally {
                            ...
                            <div className="flex flex-col items-center">
                              <div className="flex items-center">
                                ...
                                <input
                                  ...
                                  value={item.quantity}
                                  onChange={(e) =>
                                    handleQuantityInputChange(item.productId, e)
                                  }
                                  min={1}
