...
const AgencyOrders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await fetch(
        "https://minhlong.mlhr.org/api/orders/my-orders",
        {
          headers: {
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      const response = await fetch(
        `https://minhlong.mlhr.org/api/orders/${orderToCancel}/cancel`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div className="m-4">
        <div className="mb-4">
          ...
          <p className="text-gray-500 mt-1">
            Xem lại đơn hàng mà bạn đã yêu cầu
          </p>
        </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-gray-500">
                    ...
                    {Math.min(indexOfLastItem, filteredOrders.length)} của{" "}
              ...
              {orderToPayment && (
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <span className="col-span-3 font-medium">
                    {orderToPayment.finalPrice.toLocaleString("vi-VN")} đ
                  </span>
                </div>
              )}
            </div>
