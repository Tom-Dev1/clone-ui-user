...
export function SiteHeader({ isHomePage = false }: SiteHeaderProps) {
  ...
  const displayName = userDetails?.name || userDetails?.name || "User";
    ...
    async function fetchCategories() {
      try {
        ...
        setMenuItems((prevItems) =>
          prevItems.map((item) =>
            item.title === "SẢN PHẨM" ? { ...item, items: categoryItems } : item
          )
        );
      } catch (error) {
              ...
              {item.items ? (
                <>
                  ...
                  >
                    ...
                    <ChevronDown
                      className={cn(
                        "ml-1 h-4 w-3 transition-transform",
